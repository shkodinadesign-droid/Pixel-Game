...
// --- Анимация питья кофе (после "Готово!") ---
if (drink_phase) {
    drink_timer--;
    if (drink_timer <= 0) {
...
if (cook_done && cook_done_timer > 0) {
    ...
    if (cook_done_timer == 0) {
        // Списываем ингредиенты и добавляем результат
        ...
        if (variable_struct_exists(_r, "result")) {
        ...
        // Кофе для Майли — пропускаем анимацию питья Макс
        if (variable_global_exists("miley_waiting_for_coffee") && global.miley_waiting_for_coffee) {
            global.miley_waiting_for_coffee = false;
            global.miley_coffee_ready       = true;
            global.control_locked           = false;
            instance_destroy();
            exit;
        }

        // Обычный режим — Макс пьёт кофе
        drink_phase = true;
        drink_timer = 120;
        if (instance_exists(obj_max)) {
...
if (cook_anim == 0 && !cook_done && click_press) {
    ...
    var _nav_y  = tab_y;
    var _nav_h  = tab_h + 4;
    var _count  = array_length(coffee_recipes);
